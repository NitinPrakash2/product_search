<template>
  <div class="bg-gray-50/50 dark:bg-[#0b1120] min-h-screen pb-20 font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300 antialiased selection:bg-blue-100 selection:text-blue-900">
    
    <div class="flex max-w-[1440px] mx-auto w-full items-start relative">
      ...
      <div class="flex-1 w-full min-w-0 px-4 md:px-6 py-6 md:py-8">
          ...
          >
            ...
            <div class="p-2.5 pb-0">
              ...
              >
                ...
                <span v-if="getDiscount(item.document) > 0" class="absolute top-2 left-2 bg-red-500/90 backdrop-blur-md text-white text-[10px] md:text-xs px-2 py-1 rounded-md font-bold shadow-sm tracking-wide z-20">
        ...
        <div v-else class="flex flex-col items-center justify-center py-24 px-4 bg-white dark:bg-[#1e293b] rounded-3xl border border-gray-100 dark:border-slate-800/60 shadow-sm mt-4">
          <div class="w-20 h-20 bg-blue-50 dark:bg-slate-800 rounded-full flex items-center justify-center text-3xl mb-5 shadow-inner">🔍</div>
          <h3 class="text-xl font-extrabold text-slate-900 dark:text-white mb-2">No results found</h3>
          <p class="text-slate-500 dark:text-slate-400 mb-8 text-center max-w-md text-sm leading-relaxed">We couldn't find anything matching your search. Try adjusting the filters or use broader keywords to find what you're looking for.</p>
          <button class="bg-blue-600 text-white px-8 py-3 rounded-xl font-semibold shadow-lg shadow-blue-500/20 hover:bg-blue-700 active:scale-95 transition-all" @click="clearAllAndSearch">
            Clear All Filters
          </button>
        </div>
    ...
    v-bind="{
      ce_file:_var.cart_popup.ce_file
    }" />
  </div>
</template>
...
const _var = ref({
  cart_popup: {
    ce_file: _$p.data.curr.data.cart_popup[`ce_file`]
  }
});
...
const filterCb: _$cb_TYP = {
  change: (eventData: any) => {
    // 1. Capture the exact string emitted by the ProductFilter component
    if (eventData && eventData.filter_by !== undefined) {
      activeFilterByString.value = eventData.filter_by;
    }
...
// ==========================================
// 🚀 DYNAMIC DATA FETCHING (ROBUST FILTERING)
// ==========================================
const fetchProducts = async () => {
  try {
    const urlParams = new URLSearchParams(window.location.search);
    
    // 1. Sync the search input if standard text query is present
    if (!searchQuery.value && urlParams.has('q')) {
      searchQuery.value = urlParams.get('q') || '';
    }

    // 2. Build exactly the format required by your API backend
    let q = searchQuery.value.trim() || '_';
    let filterQueryStr = `?f_raw_text=${encodeURIComponent(q)}`;
    
    // Define special URL params that shouldn't be treated as catalog filters
    const specialParams = ['sattr_k', 'sattr_v', 'page', 'per_page', 'sort'];

    // Safely construct the query string for ALL parameters
    urlParams.forEach((value, key) => {
      if (key === 'q') return; 

      if (specialParams.includes(key)) {
        filterQueryStr += `&${encodeURIComponent(key)}=${encodeURIComponent(value)}`;
      } else {
        const selectedValues = value.split(','); 
        selectedValues.forEach(val => {
          filterQueryStr += `&filter=${encodeURIComponent(key)}:${encodeURIComponent(val)}`;
        });
      }
    });
    ...
    // --- STEP 1: Get 'search_combination' from get_filter_data ---
    const filterRes = await axios.post(
      `${baseUrl}?typ=get_filter_data`, 
      { query: filterQueryStr }, 
      { headers: { 'Authorization': `Bearer ${TOKEN}` } }
    );

    if (!filterRes.data.success || !filterRes.data.search_combination) {
      hasMore.value = false;
      loading.value = false;
      return;
    }
    ...
    // Enforce the current frontend pagination state
    searchCombination.page = page.value;
    searchCombination.per_page = 20;

    // 🚀 THE MAGIC FIX: Inject our specific string to replace 'null'
    if (activeFilterByString.value) {
      searchCombination.filter_by = activeFilterByString.value;
    } else {
      // If there's no filter, delete it so Typesense doesn't crash on null
      delete searchCombination.filter_by; 
    }

    // --- STEP 2: Pass search_combination into get_product_list ---
    const prodRes = await axios.post(
      `${baseUrl}?typ=get_product_list`,
      searchCombination, 
      { headers: { 'Authorization': `Bearer ${TOKEN}` } }
    );

    if (prodRes.data.success && prodRes.data.data && prodRes.data.data.products?.length > 0) {
      await new Promise(r => setTimeout(r, 300)); 
      const newItems = prodRes.data.data.products;
      
      for (const curr of newItems) {
        products.value.push({ 
          document: curr,
          images: getImages(curr), // Fetch array of all images
          activeIdx: 0,            // Track which image is showing
          interval: null           // Track the auto-scroll timer
        });
      }
      loading.value = false;
    } else {
...
const getImages = (doc: any) => {
  try {
    ...
    if (Array.isArray(images) && images.length > 0) {
      const extractedUrls = images.map((img: any) => img.url).filter((url: string) => url && !url.includes('example')).slice(0, 5); 
...
const startHoverCycle = (item: any) => {
  ...
  item.interval = setInterval(() => {
    item.activeIdx = (item.activeIdx + 1) % item.images.length;
  }, 1200); 
};

const stopHoverCycle = (item: any) => {
  if (item.interval) {
...
const setActiveImage = (item: any, index: number) => {
  item.activeIdx = index;
  if (item.interval) {
    clearInterval(item.interval);
    item.interval = null;
  }
...
// --- Mobile Swipe Variables & Handlers ---
const touchStartX = ref(0);
const touchEndX = ref(0);
const swipeThreshold = 40; // Minimum px distance to be considered a swipe
...
const handleTouchEnd = (e: TouchEvent, item: any) => {
...
const handleSwipe = (item: any) => {
  ...
  if (diff > swipeThreshold) {
    ...
    item.activeIdx = item.activeIdx > 0 ? item.activeIdx - 1 : item.images.length - 1;
  } else if (diff < -swipeThreshold) {
    // Swipe Left -> Show Next Image
    item.activeIdx = item.activeIdx < item.images.length - 1 ? item.activeIdx + 1 : 0;
  }
};
...
// --- OTHER HELPERS ---
const getDiscount = (doc: any) => {
  const p = doc?.variant_prices?.[0] || 0;
  const m = doc?.variant_mrp?.[0] || 0;
...
const addToCart = (_v:any) => {
  ...
  cartPopupRef.value.ce_call("msg", {
    type: "product:open",
    custom: { 
        product_id: _v.document?.slug || _v.document?.id, 
    },
    _p: {}, 
    _$p: {} 
  });
